...
function clickSubMenu1(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getBusinessList");
}
...
function clickSubMenu2(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getComment");
}
...
function clickSubMenu3(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getOrders");
}
